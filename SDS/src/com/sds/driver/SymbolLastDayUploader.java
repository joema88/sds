...
public class SymbolLastDayUploader {

	public static void processDailySummaryScore(String date, String stock) {
		try {
	...
	public static void main(String[] args) {
		...
		do {
			readLoadRecords(path, cDate, stock);
			System.out.println("Process daily summary and PT9...");
			processDailySummaryScore(cDate, stock);
			try {
				Thread.sleep(10000);
			} catch (Exception ex) {
	...
	public static void readLoadRecords(String path, String cDate, String stock) {
		...
		for (int k = 0; k < files.length; k++) {
			...
			try {
				if (nextFile.indexOf(cDate) >= 0) {
					...
					CSVReader.uploadCSVtoDB(path, nextFile, stock);
				}
			} catch (Exception ex) {
				ex.printStackTrace(System.out);
			}
